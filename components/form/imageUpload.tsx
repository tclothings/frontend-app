...
export default function ImageUpload({
  ...
  const handleImageLoad = (e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    const size = Math.min(naturalWidth, naturalHeight, outputSize);
  ...
  return (
    <>
      <div
        onClick={handleFileInputClick}
        className={clsx("w-full mb-7 file-upload-container cursor-pointer", {
          error: errors[name],
          "file-upload-container-success": fileName,
        })}
      >
        <label
          className={clsx("file-upload-placeholder text-gray-400", {
        ...
        <input
          ...
          ref={(el) => {
            fileInputRef.current = el;
            methods.register(name).ref(el);
          }}
          onChange={(e) => {
            methods.register(name).onChange(e);
            handleChange(e);
          }}
          accept={accept}
        />
        <ErrorMessage
          ...
          name={name}
          render={({ message }) => (
            <p className="error-message mt-3 text-sm text-red-500">{message}</p>
          )}
        />
      </div>
